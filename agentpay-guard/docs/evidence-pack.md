...
- `mandate`: authority scope, allowlists, denylists, thresholds, expiry, signer, Ed25519 mandate hash, and signature metadata.
- `payment_request`: normalized payment request, including idempotency key, decision TTL, and request hash.
- `decision`: deterministic policy result, reason, checked rules, triggered rules, screening result, and policy version.
...
- `mandate_integrity`: compares the decision-time mandate hash with the current mandate and verifies the mandate signature.

## Honest Scope

The evidence pack can show that an agent was linked to a principal, signed under a mandate, checked against deterministic payment rules and demo screening hooks, and recorded with an audit chain. Demo signing private keys are not exported. This does not prove real-world company existence, sanctions clearance, legal authorisation, or regulatory compliance.
